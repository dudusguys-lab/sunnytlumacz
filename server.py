...
print("Ładowanie/pobieranie modelu językowego Vosk (Polski)...")
# Zamiana na lang="pl" – Vosk sam pobierze model w chmurze przy pierwszym uruchomieniu
model = Model(lang="pl") 
...
async def process_audio(websocket):
    print("[+] Połączono z pluginem Minecraft.")
    ...
    try:
        ...
        print("[-] Rozłączono z pluginem Minecraft.")
...
async def main():
    ...
    async with websockets.serve(process_audio, "0.0.0.0", port):
        print("\n=== SERWER TRANSLATORA GOTOWY ===")
        print(f"Nasłuchiwanie na adresie: ws://0.0.0.0:{port}")
